...
2. **Task Characteristics**：对每个 task $a_{n}^{u} \phi$ ，请求VM的类型标记为 $a_{n}^{u} K$ ，它的估计执行时间为 ${_n^uL}$ ，通过 **Nephele** 近似方法可以推理。用户规定的 task 截止时间 ${_n^uT_{ddl}}$ ，CSP 调度任务开始时间 ${_n^uT_{start}}$ 。如果任务不能使用有限的资源在给定的 ${_n^uT_{ddl}}$ 时间完成，该 task 就会被立即拒绝：$_n^uT_{start} + _n^uL<_n^uT_{ddl}$。
